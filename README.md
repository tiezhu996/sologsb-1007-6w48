...
- 可交接的校订班次：开始前登记校订人并冻结当前轨道快照，班次内改动全部挂在本人名下；结束班次时逐条核对新旧内容，确认后写入项目历史，交接单可导出为 `.txt`。
- 未结束的班次随草稿保存，刷新、断网或关闭页面后再次打开自动恢复；另一标签页先结束同一班次时，本页会提示冲突并可选择保留本页或载入对方内容。
- 「校订历史」页按文本轨道和校订人筛选已结束的班次，可展开查看每个片段的新旧内容并再次导出交接单。
...
校订班次使用三个独立的本地键：进行中的班次（`sologsb-1007-active-shift-v1`）、已结束班次历史（`sologsb-1007-shift-history-v1`，最多保留 200 条）和最近登记的校订人姓名。结束班次时的项目快照会随草稿一起保存，供另一标签页“载入对方内容”。
